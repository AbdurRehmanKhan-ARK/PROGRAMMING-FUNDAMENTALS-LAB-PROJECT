...
│
├── 📄 main_project.c → Main Menu & Application Entry
├── 📄 registration.c → User Sign Up & Login
├── 📄 admin.c → Admin Panel & Flight Management
├── 📄 user.c → Booking, Modifying & Cancelling Flights
├── 📄 validation.c → Validations for Input & Availability
│
├── 📁 Data Files/
│ ├── ✈️ flight.txt → Flight Database
│ ├── 👤 user_info.txt → User Credentials
│ ├── 🧾 user_bookings.txt → User-specific Reservations
